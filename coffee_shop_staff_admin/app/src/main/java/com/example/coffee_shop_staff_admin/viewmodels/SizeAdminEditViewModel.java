...
public class SizeAdminEditViewModel extends BaseObservable {
    public void updateParameter(Size size)
    {
        setName(size.getName());
        DecimalFormat formatter = new DecimalFormat("###0.##");
        String formattedPrice = formatter.format(size.getPrice());
        setPrice(formattedPrice);
        getImageSource().postValue(Uri.parse(size.getImage()));
        setHasImage(true);
        setButtonText("Lưu");
    }
    @Bindable
    ...
    @Bindable
    private String buttonText = "Tạo size";
